      ...
      <div className={classes.swiper}>
        ...
        <div className={`max-w-5xl mx-auto text-center ${classes.swiperDiv}`}>
          ...
          >
            {groupedPartners.map((group, index) => (
              <SwiperSlide key={index} className="flex justify-center">
                <div className={classNames("grid grid-cols-1 gap-6",classes.imgDiv)}>
                  {group.map((src: any, idx: number) => (
                    <div key={idx} className={"bg-white p-6 rounded-xl shadow-lg w-60 flex items-center justify-center"}>
                      <img src={src} alt={`Partner ${index * 2 + idx + 1}`} className="w-20 h-auto object-contain" />
                      <img src={src} alt={`Partner ${index * 3 + idx + 2}`} className="w-20 h-auto object-contain" />
                    </div>
                  ))}
                </div>
              </SwiperSlide>
